...
deferred steps have been taken, one at a time, each with its own falsifier and off-switch:
  ...
  world:  viewer --santana / app.py wire her in; offer() is STAGE ONE of the top-down loop
          (§5.18): her settled line enters the town's LORE channel only -- 2 souls, low
...
# --- the runner that used to live here is GONE (2026-07-03 port-prep audit) -------------------
# `_watch()` / `main()` were the original way to run her against a living town -- superseded,
# line for line, by the persistent santana_app (run.py carries the same wheel/speech threads
# PLUS the saved life, the life lock, and the fail-loud reporters this file's copy never got).
# Three copies of one runner is how the frozen-world bug hid for 171k ticks; now there is one:
#
#   python -m santana_app.run        # her town, persistent, guarded
#   python -m santana_app.talk       # speak with her
#
# This module is the LIBRARY: the Santana class, play_two_layer, and the digest helpers.
