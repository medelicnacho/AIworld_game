"""The PROMPT layer: SpeechContext and everything that turns one into words on a page.

Split out of services/llm.py in the 2026-07-03 port-prep pass: llm.py had grown to 46k
holding two different things -- HOW to reach a model (transport: Ollama/DeepSeek/Mock/
Markov/Homegrown) and WHAT to say to it (this file: the SpeechContext dataclass, the mood
and disposition vocabulary, the sanitizers, and every faculty clause from work to prajna).

The split is the port seam (ROADMAP Track G): a game engine reimplements THIS file's
behaviour in its own language -- SpeechContext is the substrate's entire speech-facing API
surface -- while transport stays engine-native. Nothing here touches the network; it is
pure text over a dataclass, stdlib only.

Compatibility: services/llm.py re-exports every public name (and the underscore helpers
older callers reach for), so `from services.llm import SpeechContext, build_system` keeps
working everywhere. New code may import from services.prompts directly.
"""
